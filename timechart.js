...
function main(data){
  ...
  data = _.sortBy(data, function(d){
    ...
    return - d['samples'].length; //-d3.sum(_.map(d['samples'], function(d){ return d[CPU] + 10}));
  });
  ...
  var x_max = d3.max(_.map(_.pluck(data,'samples'), function(d){
  ...
  _.each(data, function(process, i){
    ...
    these_io      =      _.filter(process.samples, function(d){return d[CPU] == -1});
